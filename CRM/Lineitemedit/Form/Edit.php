...
class CRM_Lineitemedit_Form_Edit extends CRM_Core_Form {
  ...
  public function assignFormVariables($params = []) {
    ...
    foreach (CRM_Lineitemedit_Util::getLineitemFieldNames() as $attribute) {
      if (in_array($attribute, $this->submittableMoneyFields)) {
        $this->_values[$attribute] = CRM_Utils_Money::formatLocaleNumericRoundedForDefaultCurrency(abs($this->_lineitemInfo[$attribute]));
      }
      else {
  ...
  public function postProcess() {
    ...
    // Fetch all line items for this contribution so the Order API receives the
    // complete set and does not discard non-edited items.
    $existingLineItems = civicrm_api3('LineItem', 'get', [
    ...
    foreach ($existingLineItems as $item) {
      if ((int) $item['id'] === (int) $this->_id) {
        // Replace with the submitted edited values.
        $lineItems[] = [
          'line_item' => [
            [
              'id' => $this->_id,
        ...
        // Keep all other line items unchanged.
        $lineItems[] = ['line_item' => [$item]];
      }
    }
